...
* **語言切換器** - 提供短碼 `[hreflang_switcher]`，支援下拉選單、清單與 `theme` 外觀組別
* **後台管理介面** - 友善的語言設定管理頁面
...
5. 使用短碼 `[hreflang_switcher]`（或帶 `theme` 參數）在前端顯示語言切換器
...
`[hreflang_switcher]` - 預設下拉選單樣式（default 外觀組別）
`[hreflang_switcher style="list"]` - 清單樣式
`[hreflang_switcher theme="dark-header"]` - 套用指定外觀組別
`[hreflang_switcher style="list" theme="footer"]` - 清單樣式 + 指定外觀組別
